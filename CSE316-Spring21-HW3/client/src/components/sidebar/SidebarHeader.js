            ...
            <WCol size="5">
                {
                    props.auth && <div className="sidebar-options">
                        <WButton className="sidebar-buttons" onClick={props.createNewList} clickAnimation="ripple-light" shape="rounded"
                          id="add-list-button" 
                        style = {(props.activeList.id === undefined) ? {pointerEvents: "auto",color:"primary"} : {pointerEvents: "none",color:"#322d2d"}}
                        color={(props.activeList.id === undefined) ? "primary" : "#322d2d"}
                        >
